...
#| fig-cap: "Scatterplot with a fitted regression line showing the relationship between D Score  and Anxiety Score."
...
```{r}
#| label: fig-bar
#| fig-cap: "Mean reaction time by condition."

##Calculate mean RT and standard error (SE) of congruent and incongruent trials. 
mean_congruent <- mean(participant_wide$congruent_mean, na.rm = TRUE)
mean_incongruent <- mean(participant_wide$incongruent_mean, na.rm = TRUE)

se_congruent   <- sd(participant_wide$congruent_mean, na.rm = TRUE) / sqrt(length(na.omit(participant_wide$congruent_mean)))
se_incongruent   <- sd(participant_wide$incongruent_mean, na.rm = TRUE) / sqrt(length(na.omit(participant_wide$incongruent_mean)))


## Create data frame  
mean_data <- data.frame(
  condition = c("Congruent", "Incongruent"),
  mean_rt = c(mean_congruent, mean_incongruent),
    se = c(se_congruent, se_incongruent)
)


# Plot bar plot
barplot <- ggplot(mean_data, aes(x = condition, y = mean_rt, fill = condition)) +
  geom_col(fill = "steelblue", width = 0.5) +
  geom_errorbar(aes(ymin = mean_rt - se, ymax = mean_rt + se), width = 0.2) +
  labs(x = "Condition", y = "Mean RT (ms)", fill = "Condition") +
  theme_minimal() 
